...
describe('FeatureTable', () => {
  ...
  it('returns null when features array is empty', () => {
    const { container } = render(<FeatureTable features={[]} runId="run_123" />);
  ...
  it('renders page screenshot link when provided', () => {
    ...
    expect(screen.getByText('Ver screenshot da página completa')).toBeInTheDocument();
  });

  it('does not render page screenshot link when not provided', () => {
    render(<FeatureTable features={mockFeatures} runId="run_123" />);
    expect(screen.queryByText('Ver screenshot da página completa')).not.toBeInTheDocument();
  });
