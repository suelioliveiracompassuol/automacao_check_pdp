...
describe("ReportClient", () => {
  it("renders FilterBar", () => {
    render(
      <ReportClient results={mockResults} runId="run_123" screenshots={[]} />,
    );
    expect(
      screen.getByPlaceholderText("Buscar por SKU, nome ou URL..."),
    ).toBeInTheDocument();
  });
  ...
  it("filters by vendor", () => {
    ...
    const avonButton = vendorButtons.find((b) =>
      b.querySelector("svg[viewBox='0 0 171 54']"),
    );
    if (avonButton) {
